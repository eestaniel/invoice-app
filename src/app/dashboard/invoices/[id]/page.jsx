...
export default function Page({params}) {
  ...
  const sendDeleteRequest = async (id) => {
    await fetch(`/api/invoices?id=${id}&uid=${currentUser?.uid}`, {
      method: 'DELETE',
    })

    return {deletedInvoice: 'success'}
  }

  const updateInvoiceListDelete = async (id) => {
  ...
  const sendPutRequestStatus = async () => {
    const status = 'status'
    await fetch(`/api/invoices?type=${status}&id=${id}`, {
      method: 'PUT',
    })

    return {updatedInvoiceDBStatus: 'success'}
  }

  const updateSelectedInvoice = async () => {
    setSelectedInvoice({...selectedInvoice, invoice_details: {...selectedInvoice.invoice_details, status: 'paid'}})
  ...
  const updateInvoiceList = async (id) => {
    ...
    const index = invoiceList.findIndex((invoice) => invoice.invoice_details.custom_id === id);
    const updatedInvoiceList = [...invoiceList.slice(0, index),
      {...invoiceList[index], invoice_details: {...invoiceList[index].invoice_details, status: 'paid'}},
      ...invoiceList.slice(index + 1)]
    setInvoiceList(updatedInvoiceList)
  ...
  return (
    <>
      {deleting ? (<div role="status" className="flex w-full min-h-screen justify-center items-center">
        <svg aria-hidden="true" className="inline w-24 h-24 text-gray-200 animate-spin dark:text-gray-600 fill-red-600"
                        ...
                        <div className="button-group flex flex-row gap-2">
                          ...
                          <AlertDialog>
                            ...
                            <AlertDialogContent className={`p-12 ${theme.table_row} ${theme.delete_border}`}>
                              <AlertDialogHeader>
                                <AlertDialogTitle className={`heading-m ${theme.text}`}>Confirm
                                  Deletion</AlertDialogTitle>
                                <AlertDialogDescription className={"body text-6-muted"}>
                                  Are you sure you want to delete invoice #{selectedInvoice.invoice_details.custom_id}?
                                  This
                                  action cannot be.
                                </AlertDialogDescription>
                              </AlertDialogHeader>
                              <AlertDialogFooter className={"mt-4"}>
                                ...
                                <AlertDialogAction
                                  className={"rounded-[1.5rem] bg-9-accent text-white hover:brightness-75"}
                    ...
                    <div
                      ...
                      <div
                        ...
                        <div className="summary_header gap-1 lg:gap-2 flex flex-col">
                          ...
                          <p
                            className={`body ${theme.table_date} uppercase `}>{selectedInvoice.invoice_details.project_description}</p>
                        </div>
                    ...
                    md:justify-between">
                      ...
                      <div className="invoice_summary-item-group w-full h-fit  rounded-[0.5rem]">
                        ...
                        <div
                          ...
                          <p
                            className="heading-m text-white">$ {convertToCurrency(selectedInvoice.invoice_details.total)}</p>
                        </div>
                      </div>
                    </div>
                  </div>
                          ...
                          <AlertDialogContent className={`p-12 ${theme.table_row} ${theme.delete_border}`}>
                            ...
                            <AlertDialogFooter className={"mt-4"}>
                              <AlertDialogCancel
                                className={`rounded-[1.5rem] ${theme.cancel_button}`}>Cancel</AlertDialogCancel>
                              <AlertDialogAction
                                className={"rounded-[1.5rem] bg-9-accent text-white hover:bg-10-soft-red"}
                                onClick={() => handleDelete(selectedInvoice.invoice_details.custom_id)}>Delete</AlertDialogAction>
                            </AlertDialogFooter>
                          </AlertDialogContent>
                        </AlertDialog>
